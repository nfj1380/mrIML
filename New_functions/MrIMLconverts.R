...
MrIMLconverts <- function (yhats,  mode='regression'){
...
#extracts data from each model
  ...
  #regression
    Predictor$new(mod, data=Y, y=Datalist[[i]]$class)
  
      if (mode=='çlassification'){
    #create a IML predictor variable for each response
      Predictor$new(mod, data=Y, y=Datalist[[i]]$class, type="prob")
      }
