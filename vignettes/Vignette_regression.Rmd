...
library(ggrepel); library(ranger); library(future.apply)
...
yhats <- mrIMLpredicts(X=X,Y=Y, model1=model1, balance_data='no', model='regression', seed = sample.int(1e8, 1)) ## Balanced data= up updamples and down downsampled to create a balanced set. For regression 'no' has to be selected.
...
```{r}
ModelPerf <- mrIMLperformance(yhats, model1, Y=Y, model='regression')
