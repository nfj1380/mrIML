---
title: "A MrIML vignette: Landscape genetics (classification) "
...
yhats_glm <- mrIMLpredicts(X=X,Y=Y, Model=model_glm, balance_data='no', mode='classification',  tune_grid_size=5, seed = sample.int(1e8, 1), racing=F, k=5 )
...
```{r, message = FALSE, warning = FALSE}
source("~/MrIML/mrIML/R/mrvip.R")
source("~/MrIML/mrIML/R/interpret_Mrvi.R")
VI <- mrVip(yhats_glm, X=X, Y=Y) 
#something isnt right...check other code
vi_plots_pca <- interpret_Mrvi(VI=VI,  X=X,Y=Y,   modelPerf=ModelPerf_glm, cutoff= 0.05, mode='classification') #the cutoff reduces the number of individual models printed in the second plot. 

outlier_SNPS <- vi_plots_pca$outLiers
outlier_SNPS
```
...
# plot_vi(VI=VI,  X=X,Y=Y, modelPerf=ModelPerf_rf, groupCov=groupCov, cutoff= 0.5)
...
mfl <- mrFlashlight(yhats_rf, X=X, Y=Y, response = "multi", mode='classification') #the global set
...
interactions <-mrInteractions(yhats= yhats_rf, feature='Forest', X=X, Y=Y,  num_bootstrap=1, top.int=2) #this is computationally intensive so multicore are needed. 

#mrPlot_interactions(interactions, X=X,Y=Y, top_ranking = 3, top_response=3)
