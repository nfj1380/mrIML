...
```{r, include=FALSE}
Y_names <- sort(colnames(Y), decreasing = FALSE)
...
for (i in 1:42) {
  names(Y)[names(Y) == Y_names[i]] <- long_names[i]
  names(data)[names(data) == Y_names[i]] <- long_names[i]
  }
names(Y) <- as.character(names(Y))
```
