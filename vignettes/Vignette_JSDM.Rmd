...
```{r}
VI <- mrVip(yhats_rf_downSamp, X)

vi_i <- interpret_Mrvi(VI=VI, modelPerf=ModelPerf_rf_downSamp, groupCov = NULL, cutoff = 0.5, mode = "classification" )
```
...
We can utilise all the bootstrapped partial dependence estimates (pds) to construct a co-occurrence network. We show how this object can be converted to an igraph object and plotted.  This is a directed network and edges are scaled by the standard deviation of the marginal change in prediction. Red are positive associations (the predicted occurrence of the taxa increases with the presence of the other) and blue are negative (the predicted occurence of the taxa decreases with the presence of the other).
