...
We can utilise all the bootstrapped partial dependence estimates (pds) to construct a co-occurence network. We show how this object can be converted to an igraph object and plotted.  This is a directed network and edges are scaled by the standard deviation of the marginal change in prediction. Red are positive associations (the predicted occurence of the taxa increases with the presence of the other) and blue are negative (the predicted occurence of the taxa decreases with the presence of the other).
...
Finally, we can quantify the importance of interactions overall as well as one and two-way interaction importance using the same bootstrap approach.See https://github.com/mayer79/hstats for more details about the approach.

```{r}

int_ <- mrInteractions(yhats=yhats_rf_downSamp, X, Y, num_bootstrap=10,
