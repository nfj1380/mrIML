---
title: 'Classification working example'
...
library(vip); library(tidymodels); library(randomForest);   library(gbm);library(pdp);
...
# Filter and balance your dataset
...
yhats <- mrIMLpredicts(X=X,Y=Y, model1=model1, balance_data='no', mode='classification', tune_grid_size= 10, seed = sample.int(1e8, 1)) ## in MrTidymodels. Balanced data= up updamples and down downsampled to create a balanced set
...
yhats <- mrIMLpredicts(X=X,Y=Y, model1=model1, balance_data='no', mod='classification', seed = sample.int(1e8, 1))
